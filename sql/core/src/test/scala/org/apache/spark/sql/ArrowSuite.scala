...
import org.apache.spark.sql.test.{ArrowTestData, SharedSQLContext}

class ArrowSuite extends QueryTest with SharedSQLContext with ArrowTestData {
  ...
  test("convert int column with null to arrow") {
    test(arrowNullInts, arrowNullIntsFile)
  }

  test("convert string column with null to arrow") {
    test(arrowNullStrings, arrowNullStringsFile)
  }

  private def test(df: DataFrame, arrowFile: String) {
